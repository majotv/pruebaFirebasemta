...
export class ServicioProductos{
    ...
    crear=(producto)=>{
            global.db.collection("productos").doc(producto.id).set(producto).then(function () {
    ...
    eliminar = (id) => {
        global.db.collection("productos").doc(id).delete().then(function () {
    ...
    actualizar=(objeto)=>{
        global.db.collection("productos").doc(objeto.id).update(
            {
